       ...
       do n=1,NUM_REACTS
          do m=1,size(rresults,2)
             ...
             do p=1,3
                ...
                else if ( (rresults(n,m,1) + rresults(n,m,2) + rresults(n,m,3)) == 0 ) then
!                   print *, "Sum = 0 in write reactions"
                   continue
                else
...
!         se_box%se_fbdnsigs(n) = greendutta(e,f,w,o,a,b)
    ...
    DO n=1,SIZE(se_box%se_o_ionst)
       ASSOCIATE ( en  => se_box%se_energy           , &
            ...
            j   => se_box%se_o_ionst(n)%j_ion   , &
            jb  => se_box%se_o_ionst(n)%jb_ion  , &
            jc  => se_box%se_o_ionst(n)%jc_ion  , &
            gs  => se_box%se_o_ionst(n)%gams_ion, &
            gb  => se_box%se_o_ionst(n)%gamb_ion, &
            ts  => se_box%se_o_ionst(n)%ts_ion  , &
