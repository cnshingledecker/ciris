...
CONTAINS
  FUNCTION branching(reactant1,reactant2,pr)
    ...
    firstcall: IF ( pr .EQ. 1 ) THEN
      ! Bump count of irradiation reactions:
      !IF ( (reactant1 .EQ. CRPNUM) .OR. (reactant1 .EQ. EXCNUM) ) THEN
      IF ( (reactant1 .EQ. EXCNUM) ) THEN
        IF ( reactant2 .EQ. ONUM ) THEN
          CUIRCT(1) = CUIRCT(1) + 1
        ELSE IF ( reactant2 .EQ. O2NUM ) THEN
          CUIRCT(2) = CUIRCT(2) + 1
        ELSE IF ( reactant2 .EQ. O3NUM ) THEN
          CUIRCT(3) = CUIRCT(3) + 1
        END IF
      !ELSE IF ( (reactant2 .EQ. CRPNUM) .OR. (reactant2 .EQ. EXCNUM) ) THEN
      ELSE IF (  (reactant2 .EQ. EXCNUM) ) THEN
        IF ( reactant1 .EQ. ONUM ) THEN
          CUIRCT(1) = CUIRCT(1) + 1
        ELSE IF ( reactant1 .EQ. O2NUM ) THEN
          CUIRCT(2) = CUIRCT(2) + 1
        ELSE IF ( reactant1 .EQ. O3NUM ) THEN
          CUIRCT(3) = CUIRCT(3) + 1
        END IF
      END IF
